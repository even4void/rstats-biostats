...
<div class="outline">
Le modèle de régression de Cox est un modèle semi-paramétrique simple dans lequel on modélise la fréquence instantanée de l'événement d'intérêt, appelé "hazard", en fonction de variables explicatives sous la forme : 
$$ \log\big(h(t;x_1,x_2,\dots)\big) = \log\big(h_0(t)\big) + \beta_1x_1 + \beta_2x_2 + \dots $$
</div>
...
Pour obtenir la médiane de survie dans deux groupes, on remplacera le terme d'intercept (`~ 1`) par le facteur correspondant. En réalité, `survfit()` permet de construire la table de mortalité, que l'on peut afficher entièrement ou partiellement en appliquant la méthode `summary()` :
```{r}
