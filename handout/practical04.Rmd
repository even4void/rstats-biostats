...
Les paramètres suivants doivent être définis dans R ou RStudio afin de reproduire les analyses présentées dans ce document. Les packages `ggplot2`{.pkg}, `ggfortify`{.pkg}, `hrbrthemes`{.pkg}, `directlabels`{.pkg}, `cowplot`{.pkg}, `texreg`{.pkg}, `lme4`{.pkg} et `geepack`{.pkg} ne font pas partie de la distribution R et doivent être installés si nécessaire :
...
1. Estimer les paramètres d'un modèle GEE avec une matrice de variance-covariance non stucturée (`unstructured`) ou "exchangeable" et un terme d'interaction `age:smoke`. Attention, la variable étant binaire, il est nécessaire de préciser une famille de type `binomial`. Le statut fumeur de la mère est-il significatif au seuil conventionel de 5 % ? Même question pour l'âge.
