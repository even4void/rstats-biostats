...
Une représentation graphique possible est données ci-après.
```{r}
...
Voici différents modèles d'analyse de variance, dont un modèle d'ANOVA à mesures répétées (modèle `m3` ci-dessous) :
```{r}
...
Et voici une approche par modèle de régresison à effets aléatoires, ici un intercept aléatoire pour chaque sujet :
```{r}
...
Visuellement, le modèle à intercept aléatoire se traduit par des profils individuels en tous points identiques à l'exception du niveau moyen : 
```{r}
yhat <- predict(m)
