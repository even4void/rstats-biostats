...
Une représentation graphique sous forme de moyennes conditionnelles s'obtient assez facilement avec **ggplot2**. Le calcul des moyennes et des intervalles de confiance à 95 % est réalisé avec **Hmisc** (attention, `Hmisc::summarize()` n'accepte pas de notation par formule, contrairement à la fonction `summary()` du même package, qui s'appelle en fait `summary.formula()`):
```{r}
...
L'alternative consiste à utiliser des tests post-hoc de type Tukey HSD à l'aide de `TukeyHSD()`, ou de l'alternative disponible _via_ la fonction `multcomp::glht()`. Voici une illustration avec le package **multcomp**, plus flexible et plus générale que la fonction de base de R. À noter qu'il est également nécessaire de travailler avec le terme d'interaction et d'utiliser `lm()` au lieu de `aov()` :
```{r}
